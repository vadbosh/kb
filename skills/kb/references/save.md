...
**`route` and `sync` print a `⚠` line per condition that is wrong about the
entry point, and name `references/entry-point.md` beside them. Saw one — read
that file and fix what it lists, in the same turn.** Empty `kb:fill` slots are
there too: the save half fills them from the session, it does not hand them back.

No `⚠` line means there is nothing wrong with the entry point, and that page
costs a quarter of this one — do not open it on spec.
