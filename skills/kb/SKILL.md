...
| a `⚠` line about the entry point, from any command that printed one | `references/entry-point.md`, in addition |
...
the alternatives; `references/save.md` says what to do with that refusal.
...
asked. One exception, and it is not a human's text: a `<!-- kb:fill … -->`
comment is kb's own placeholder for an answer nobody has given yet. The save
half replaces those with the answer, the same turn it writes the notes.
